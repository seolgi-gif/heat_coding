...
try: 
    ...
    nanum_gothic = next((f for f in font_path if 'NanumGothic' in f), 
None) 
...
# --- 2. 2D 열전달 시뮬레이션 함수 --- 
# (이전과 동일, 안정성 높음) 
def run_2d_heat_simulation(k, L_x, rho, cp=1000, T_hot=1000+273.15, 
T_initial=20+273.15, sim_time_minutes=15): 
...
/ dx**2 
        ...
        if time_to_target is None and current_inner_temp_k >= 
TARGET_TEMP_KELVIN: 
...
st.title("
 🌡
 2D 열전달 시뮬레이션") 
st.markdown("외부 1000°C 환경에서 **15분** 동안, 재료의 **두께**에 따라 
내부 온도가 어떻게 변하는지 관찰합니다.") 
 
st.sidebar.header("
 ⚙
 시뮬레이션 설정") 
selected_material_name = st.sidebar.selectbox("1. 재료 선택", 
options=list(scenarios.keys())) 
thickness_cm = st.sidebar.slider("2. 재료 두께 (cm)", min_value=1.0, 
max_value=20.0, value=5.0, step=0.5) 
 
thickness_m = thickness_cm / 100.0 
material_props = scenarios[selected_material_name] 
k = material_props['k']; rho = material_props['rho']; cp = 
material_props['cp'] 
...
if st.sidebar.button("
 🚀
 시뮬레이션 실행"): 
    with st.spinner(f"'{selected_material_name}'(두께: 
{thickness_cm}cm)으로 {SIMULATION_TIME_MINUTES}분간 시뮬레이션 중..."): 
        time_pts, temp_hist, final_temp_dist, time_to_target = 
run_2d_heat_simulation( 
            k=k, L_x=thickness_m, rho=rho, cp=cp, 
sim_time_minutes=SIMULATION_TIME_MINUTES 
        ) 
 
    st.subheader(f"
 📊
 {SIMULATION_TIME_MINUTES}분 시뮬레이션 결과") 
     
    with st.expander("
 🔬
 선택 재료의 물리적 특성 보기"): 
        st.markdown(f"- **열전도율 (k)**: `{k}` W/m·K (낮을수록 단열 성능 
좋음)") 
    ...
    if time_pts is None: 
        ...
        col1.metric(f"최종 온도 ({SIMULATION_TIME_MINUTES}분 후)", 
f"{final_temp:.1f} °C") 
         
        if final_temp < 120: 
             col2.metric("목표(120°C) 달성", "
 ✅
 성공") 
        else: 
             col2.metric("목표(120°C) 달성", "
 ...
 실패") 
        ...
        if time_to_target is not None: 
            ...
            col3.metric("120°C 도달 시간", f"{SIMULATION_TIME_MINUTES}분 
이상") 
 
        # --- 5. 결과 시각화 --- 
        fig1, ax1 = plt.subplots(figsize=(10, 5)) 
        ax1.plot(time_pts / 60, temp_hist, 
label=f"{selected_material_name} ({thickness_cm}cm)", lw=2.5) 
        ax1.axhline(y=120, color='r', linestyle='--', label='목표 최대 
온도 (120°C)') 
         
        ax1.set_title(f'내부 표면 온도 변화', fontproperties=font_prop, 
fontsize=16) 
...
ax1.set_xlim(0, SIMULATION_TIME_MINUTES) 
        max_temp_visual = max(temp_hist) 
        ax1.set_ylim(15, max(150, max_temp_visual * 1.2)) 
        st.pyplot(fig1) 
 
        fig2, ax2 = plt.subplots(figsize=(10, 3)) 
        im = ax2.imshow(final_temp_dist, cmap='inferno', aspect='auto', 
extent=[0, thickness_cm, 0, 10], vmin=20, vmax=1000) 
        fig2.colorbar(im, ax=ax2, label='온도 (°C)'); 
ax2.set_title(f'최종 시간에서의 2D 온도 분포', fontproperties=font_prop, 
fontsize=16) 
...
ax2.set_ylabel('높이 방향 (cm)', fontproperties=font_prop) 
    ...
    st.info("사이드바에서 재료와 두께를 설정한 후 '시뮬레이션 실행' 버튼을 
눌러주세요.")
